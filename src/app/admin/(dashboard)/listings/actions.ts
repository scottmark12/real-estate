...
export async function upsertListing(formData: FormData) {
  ...
  const payload = {
    ...
    featured_headline: String(formData.get("featured_headline") ?? "").trim() || null,
    featured_excerpt: String(formData.get("featured_excerpt") ?? "").trim() || null,
    featured_focal: String(formData.get("featured_focal") ?? "").trim() || null,
